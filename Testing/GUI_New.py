...
# Base directory = lokasi file GUI ini (Testing\)
BASE_DIR = os.path.dirname(os.path.abspath(__file__))

# ============================================================
# KONSTANTA KOLOM SENSOR
# ============================================================
EXH_COLS = [
...
# ============================================================
# PREPROCESSOR: PCA → CKF → MinMax Normalize
# Parameter tetap dari thesis — Combustion +Outlier (RTF 1-5)
# ============================================================

# ── Tabel 4.4: StandardScaler (mean & std per sensor) ───────
_SS_MEANS = np.array([1159.8593, 1170.6201, 1147.2312, 1177.0129, 1188.4866])
_SS_STDS  = np.sqrt([722.5864, 818.3638, 799.4549, 940.6627, 854.7322])

# ── Tabel 4.6: PCA PC1 loadings (eigenvector) ───────────────
_PCA_LOADINGS = np.array([0.455067, 0.448755, 0.447310, 0.436433, 0.448300])

# ── Tabel 4.7: MinMax scaler (Combustion +Outlier) ──────────
_MM_MIN =  -4.8861
_MM_MAX =   2.1413


class StrictMonotonicCKF:
    """Identik dengan dataset.py — dipakai di pipeline preprocessing."""
...
class Preprocessor:
    """
    Pipeline preprocessing sensor data mentah → Health Index (HI).

      1. Pilih kolom EXH_COLS (Exhaust Cyl #1–5)
      2. StandardScaler (fixed params, Tabel 4.4)
      3. PCA (fixed loadings, Tabel 4.6) → PC1 raw
      4. StrictMonotonicCKF smoothing (meas_var=15.0)
      5. MinMax (fixed params, Tabel 4.7) → HI output

    Parameter CKF dapat diubah via Settings.
    """

    def __init__(self, ckf_meas_var: float = 15.0, ckf_proc_var: float = 0.001,
                 ckf_growth_rate: float = 1.01):
    ...
    def run(self, df: pd.DataFrame) -> list:
        """
        Input  : DataFrame dengan minimal kolom EXH_COLS
        Output : list HI (float), panjang = len(df)
        """
        ...
        # ── 1. StandardScaler (fixed) ────────────────────────────
        X_scaled = (X - _SS_MEANS) / _SS_STDS

        # ── 2. PCA (fixed loadings) → PC1 raw ───────────────────
        pc1_raw = X_scaled @ _PCA_LOADINGS
        ...
        # ── 4. MinMax (fixed, Tabel 4.7) ─────────────────────────
        hi = (pc1_filtered - _MM_MIN) / (_MM_MAX - _MM_MIN)
...
class MLPipeline:
    ...
    def run_inference(self, hi_data):
        """
        Input  : hi_data — list/array HI (0–1), hasil Preprocessor.
...
class SettingsDialog(tk.Toplevel):
    def __init__(self, parent, settings, on_save):
        ...
        fields = [
            ...
            ("Model Dir:",               "model_dir"),
            ("CKF Meas Var (R):",         "ckf_meas_var"),
            ("CKF Proc Var (Q):",         "ckf_proc_var"),
            ("CKF Growth Rate:",          "ckf_growth_rate"),
        ]
        self.entries = {}
    ...
    def _save(self):
        try:
            ...
            self.settings['ckf_meas_var']    = float(self.entries['ckf_meas_var'].get())
            self.settings['ckf_proc_var']    = float(self.entries['ckf_proc_var'].get())
            self.settings['ckf_growth_rate'] = float(self.entries['ckf_growth_rate'].get())
            self.on_save()
...
class RULPredictorGUI:
    def __init__(self, root):
        ...
        self.last_csv_data   = None   # HI hasil preprocessing (full length)
    ...
    def load_settings(self):
        default = {
            ...
            "model_dir":         os.path.join(BASE_DIR, "Training Results"),
            "ckf_meas_var":      15.0,
            "ckf_proc_var":      0.001,
    ...
    # ------------------------------------------------------------------
    # UI construction  (tidak berubah dari versi asli)
    # ------------------------------------------------------------------
    def _build_ui(self):
        # ── TOP TOOLBAR ────────────────────────────────────────────────
    ...
    def _mode_load_csv(self):
        ...
        try:
            ...
            # ── Cek apakah CSV punya kolom exhaust (multi-sensor) ────
            has_exh_cols = all(c in df.columns for c in EXH_COLS)

            if has_exh_cols:
                # Pipeline lengkap: PCA → CKF → normalize
                hi_full = self.preprocessor.run(df)
            else:
                # Fallback: CSV sudah berisi kolom HI tunggal
                num_cols = df.select_dtypes(include=[np.number]).columns
                if len(num_cols) > 0:
                    hi_full = df[num_cols[0]].values.tolist()
                else:
                    df2 = pd.read_csv(path, header=None)
                    hi_full = pd.to_numeric(df2.iloc[:, 0],
                                            errors='coerce').dropna().tolist()

        except Exception as e:
            tk.messagebox.showerror("Error loading CSV", str(e))
            return
